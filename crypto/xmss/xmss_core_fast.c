                    ...
                    xmss_progress_cb cb, void *cb_userdata)
{
    ...
    *smlen += params->n;
    ...
    s_wots_sign(params, sm, msg_h, sk_seed, pub_seed, ots_addr);
                        ...
                        unsigned char *pk, unsigned char *sk)
{
    ...
    for (i = 0; i < params->d - 1; i++) {
        ...
        s_wots_sign(params, wots_sigs + i*params->wots_sig_bytes, pk, sk + params->index_bytes, pk+params->n, addr);
    }
    ...
    *smlen += params->tree_height*params->n;
    ...
    for (i = 0; i < params->d; i++) {
        ...
        if (! (((idx + 1) & ((1ULL << ((i+1)*params->tree_height)) - 1)) == 0)) {
            ...
            s_wots_sign(params, wots_sigs + i*params->wots_sig_bytes, states[i].stack, sk_seed, pub_seed, ots_addr);
