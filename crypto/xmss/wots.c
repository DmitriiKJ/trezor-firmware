                                          ...
                                          const unsigned char *msg, uint32_t ctr)
{
    ...
    base_w(params, lengths, params->wots_len, digest);
 ...
 */
static uint32_t wots_grind(const xmss_params *params, int *lengths,
               ...
               uint32_t addr[8])
{
    ...
    ctr = wots_grind(params, lengths, msg);
 ...
 *
 * Writes the computed public key to 'pk'.
 * Returns -1 if the signature's counter does not satisfy the WOTS+C constant
 * sum, 0 otherwise.
 */
int s_wots_pk_from_sig(const xmss_params *params, unsigned char *pk,
                      ...
                      const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    /* WOTS+C: recover the grinding counter and recompute the chain lengths.
       The constant digit sum takes the place of the WOTS+ checksum, so it must
       be enforced here: without this check an attacker could grind a counter
       whose digits all exceed those of a signed message and, since chains only
       ever advance forwards, forge from a single signature. */
