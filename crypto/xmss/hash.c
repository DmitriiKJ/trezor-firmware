...
#define XMSS_HASH_PADDING_WOTS_C 5
...
/*
 * WOTS+C message digest: hashes the n-byte message together with the 4-byte
 * grinding counter. The output is interpreted in base_w to obtain the chain
 * lengths, whose digits must sum to params->wots_target_sum.
 */
int wots_c_hash(const xmss_params *params, unsigned char *out,
                const unsigned char *msg, uint32_t ctr)
{
    unsigned char buf[params->padding_len + params->n + XMSS_WOTS_CTR_BYTES];

    ull_to_bytes(buf, params->padding_len, XMSS_HASH_PADDING_WOTS_C);
    memcpy(buf + params->padding_len, msg, params->n);
    ull_to_bytes(buf + params->padding_len + params->n,
                 XMSS_WOTS_CTR_BYTES, ctr);

    return core_hash(params, out, buf,
                     params->padding_len + params->n + XMSS_WOTS_CTR_BYTES);
}
