...
START_TEST(test_shrincs_stateless_verify) {
    ...
    ck_assert_int_eq(shrincs_verify(message, 32, sl_signature, SL_SIZE, &pk), 1);
}
END_TEST
...
START_TEST(test_shrincs_stateful_verify) {
    ...
    ck_assert_int_eq(shrincs_verify(message, 32, sf_signature, sf_size, &pk), 1);
}
END_TEST
...
START_TEST(test_shrincs_stateless_sign) {
    ...
    ck_assert_int_eq(shrincs_sign_stateless(message, 32, &sk, sl_signature), 1);
...
START_TEST(test_shrincs_stateful_sign) {
    ...
    ck_assert_int_eq(shrincs_sign_stateful(message, 32, &sk, &state, sf_signature), 1);
